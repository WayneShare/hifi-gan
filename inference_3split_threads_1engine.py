...
def getSlice(melData, slices, index):
    step = int(melData.shape[2]/slices)
    star = index*(step)-overlap_frames
    if star<0:
        star=0
    end = (index+1)*(step)
    if index==slices-1 or end > melData.shape[2]:
        end = melData.shape[2]
...
class DataProducer(threading.Thread):
    ...
    def run(self):
        for index in range(slices):
            ...
            # 
            print ("%s: %s is producing %d to the queue!" %(time.ctime(), self.getName(), index))
...
def slicesProcess(generator, melData):
...
def inferOneFile(argsAll, generator, filname):
    ...
    global all_array
    all_array, overlap_frames = slicesProcess(generator, melData)
